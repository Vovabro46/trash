--[[
    GILA MONSTER LIBRARY [SOURCE v8]
    - Fixed "Black Menu" issue (ZIndex Corrected)
    - Full Dynamic Recoloring
    - Mobile Support & Configs Included
]]
...
Library.ThemePresets = {
    ["Default"] = { 
        Main = Color3.fromRGB(20, 20, 20), Sidebar = Color3.fromRGB(25, 25, 28), 
        Section = Color3.fromRGB(30, 30, 35), Accent = Color3.fromRGB(120, 110, 225),
        Text = Color3.fromRGB(255, 255, 255)
    },
    ["Blood Moon"] = { 
        Main = Color3.fromRGB(20, 15, 15), Sidebar = Color3.fromRGB(25, 20, 20), 
        Section = Color3.fromRGB(30, 25, 25), Accent = Color3.fromRGB(230, 60, 60),
        Text = Color3.fromRGB(255, 255, 255)
    },
    ["Oceanic"] = { 
        Main = Color3.fromRGB(15, 20, 25), Sidebar = Color3.fromRGB(20, 25, 30), 
        Section = Color3.fromRGB(25, 30, 35), Accent = Color3.fromRGB(0, 160, 255),
        Text = Color3.fromRGB(255, 255, 255)
    },
    ["Midnight"] = { 
        Main = Color3.fromRGB(10, 10, 15), Sidebar = Color3.fromRGB(15, 15, 20), 
        Section = Color3.fromRGB(20, 20, 25), Accent = Color3.fromRGB(80, 80, 255),
        Text = Color3.fromRGB(255, 255, 255)
    },
    ["Void"] = { 
        Main = Color3.fromRGB(5, 5, 5), Sidebar = Color3.fromRGB(10, 10, 10), 
        Section = Color3.fromRGB(15, 15, 15), Accent = Color3.fromRGB(255, 255, 255),
...
local Icons = {
    ...
    ["minimize"] = "rbxassetid://6031094670",
    ["x"] = "rbxassetid://6031094678",
    ["arrow_d"] = "rbxassetid://6031091304",
...
function Library:RegisterThemeObj(type, obj, property)
    ...
    -- Apply immediately safely
    if Library.Theme[type] then
        pcall(function() obj[property] = Library.Theme[type] end)
    end
end
...
function Library:SetTheme(name)
    ...
    if preset then
        for k, v in pairs(preset) do Library:UpdateTheme(k, v) end
        Library:Notify("Theme", "Loaded: "..name, 2)
    end
end

function Library:GetThemes()
    local t = {}
    for k in pairs(Library.ThemePresets) do table.insert(t, k) end
    table.sort(t)
    return t
end
...
local function MakeDraggable(trigger, target, isButton)
    ...
    trigger.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            dragging = false
            if isButton and not hasMoved and trigger:FindFirstChild("TouchClick") then
                trigger.TouchClick:Fire()
            end
        end
    end)
end
...
function Library:Notify(title, desc, duration)
    local Frame = Create("Frame", {Parent = NotifContainer, BackgroundColor3 = Color3.fromRGB(25,25,25), Size = UDim2.new(1,0,0,60), AutomaticSize=Enum.AutomaticSize.Y, BorderSizePixel=0, ZIndex=100})
    Create("UICorner", {Parent=Frame, CornerRadius=UDim.new(0,6)})
    local S = Create("UIStroke", {Parent=Frame, Color=Library.Theme.Accent, Thickness=1.5, Transparency=0.5}); Library:RegisterThemeObj("Accent", S, "Color")
    
    local T1 = Create("TextLabel", {Parent=Frame, Text=title:upper(), TextColor3=Library.Theme.Accent, Font=Enum.Font.GothamBlack, TextSize=13, BackgroundTransparency=1, Size=UDim2.new(1,-20,0,20), Position=UDim2.new(0,10,0,5), TextXAlignment=Enum.TextXAlignment.Left, ZIndex=101})
    Library:RegisterThemeObj("Accent", T1, "TextColor3")
    
    local T2 = Create("TextLabel", {Parent=Frame, Text=desc, TextColor3=Library.Theme.Text, Font=Enum.Font.GothamMedium, TextSize=11, BackgroundTransparency=1, Size=UDim2.new(1,-20,0,0), Position=UDim2.new(0,10,0,25), TextXAlignment=Enum.TextXAlignment.Left, TextWrapped=true, AutomaticSize=Enum.AutomaticSize.Y, ZIndex=101})
    Library:RegisterThemeObj("Text", T2, "TextColor3")
    
    Frame:TweenPosition(UDim2.new(0,0,0,0), "Out", "Back", 0.5)
    task.delay(duration or 4, function() if Frame then Frame:Destroy() end end)
end
...
function Library:Window(opts)
    ...
    -- Toggle Button
    local MobToggle = Create("ImageButton", {
        Parent = Screen, BackgroundColor3 = Library.Theme.Main, Position = UDim2.new(0,50,0,50), 
        Size = UDim2.new(0,45,0,45), Image = Icons.menu, ImageColor3 = Library.Theme.Accent, Active = true, ZIndex = 200
    })
    ...
    local MS = Create("UIStroke", {Parent=MobToggle, Color=Library.Theme.Accent, Thickness=2}); Library:RegisterThemeObj("Accent", MS, "Color")
    local TouchClick = Instance.new("BindableEvent", MobToggle); TouchClick.Name = "TouchClick"
    
    -- Main Frame
    local Main = Create("Frame", {
        Parent = Screen, BackgroundColor3 = Library.Theme.Main, Position = UDim2.new(0.5,-325,0.5,-225), 
        Size = UDim2.new(0,650,0,450), ClipsDescendants = true, ZIndex = 1
    })
    ...
    TouchClick.Event:Connect(function() Main.Visible = not Main.Visible end)
    MakeDraggable(MobToggle, MobToggle, true)
    
    -- Header
    local Top = Create("Frame", {Parent = Main, BackgroundColor3 = Library.Theme.Main, Size = UDim2.new(1,0,0,40), BorderSizePixel = 0, ZIndex=2})
    Library:RegisterThemeObj("Main", Top, "BackgroundColor3")
    
    local Logo1 = Create("TextLabel", {
        Parent = Top, Text = "GILA", TextColor3 = Library.Theme.Text, Font = Enum.Font.GothamBlack, 
        TextSize = 16, Size = UDim2.new(0,50,1,0), Position = UDim2.new(0,15,0,0), 
        BackgroundTransparency=1, TextXAlignment=Enum.TextXAlignment.Left, ZIndex=3
    })
    Library:RegisterThemeObj("Text", Logo1, "TextColor3")
    
    local Logo2 = Create("TextLabel", {
        Parent = Top, Text = "MONSTER", TextColor3 = Library.Theme.Accent, Font = Enum.Font.GothamBlack, 
        TextSize = 16, Size = UDim2.new(0,100,1,0), Position = UDim2.new(0,60,0,0), 
        BackgroundTransparency=1, TextXAlignment=Enum.TextXAlignment.Left, ZIndex=3
    })
    ...
    -- Sidebar
    local Sidebar = Create("Frame", {Parent=Main, BackgroundColor3=Library.Theme.Sidebar, Size=UDim2.new(0,150,1,-41), Position=UDim2.new(0,0,0,41), BorderSizePixel=0, ZIndex=2})
    ...
    function Tabs:Tab(name)
        local TBtn = Create("TextButton", {
            Parent = SidebarList, Size = UDim2.new(1,0,0,36), BackgroundTransparency=1, 
            Text = "    "..name:upper(), TextColor3 = Library.Theme.DimText, Font = Library.Theme.FontMain, 
            TextSize = 12, TextXAlignment = Enum.TextXAlignment.Left, AutoButtonColor=false, ZIndex=4
        })
        local Ind = Create("Frame", {Parent=TBtn, BackgroundColor3=Library.Theme.Accent, Size=UDim2.new(0,2,0,18), Position=UDim2.new(0,0,0.5,-9), Visible=false, ZIndex=5})
        ...
        local Left = Create("ScrollingFrame", {Parent=Page, Size=UDim2.new(0.48,0,1,0), BackgroundTransparency=1, ScrollBarThickness=0, AutomaticCanvasSize=Enum.AutomaticSize.Y, CanvasSize=UDim2.new(0,0,0,0), ZIndex=4})
        local Right = Create("ScrollingFrame", {Parent=Page, Size=UDim2.new(0.48,0,1,0), Position=UDim2.new(0.52,0,0,0), BackgroundTransparency=1, ScrollBarThickness=0, AutomaticCanvasSize=Enum.AutomaticSize.Y, CanvasSize=UDim2.new(0,0,0,0), ZIndex=4})
        ...
        function Funcs:Section(title, side)
            ...
            local HTxt = Create("TextLabel", {Parent=H, Text=title:upper(), TextColor3=Library.Theme.Accent, Font=Library.Theme.FontMain, TextSize=11, BackgroundTransparency=1, Size=UDim2.new(1,-10,1,0), Position=UDim2.new(0,10,0,0), TextXAlignment=Enum.TextXAlignment.Left, ZIndex=7})
            ...
            function E:Toggle(txt, def, flag, cb)
                ...
                local T = Create("TextLabel", {Parent=TogBtn, Text=txt, TextColor3=Library.Theme.Text, Font=Library.Theme.FontSmall, TextSize=12, BackgroundTransparency=1, Size=UDim2.new(1,-25,1,0), TextXAlignment=Enum.TextXAlignment.Left, ZIndex=8})
                Library:RegisterThemeObj("Text", T, "TextColor3")
                local Box = Create("Frame", {Parent=TogBtn, BackgroundColor3=Library.Theme.Main, Size=UDim2.new(0,14,0,14), Position=UDim2.new(1,-14,0.5,-7), ZIndex=8})
                Library:RegisterThemeObj("Main", Box, "BackgroundColor3"); Create("UICorner", {Parent=Box, CornerRadius=UDim.new(0,3)}); local S = Create("UIStroke", {Parent=Box, Color=Library.Theme.Border}); Library:RegisterThemeObj("Border", S, "Color")
                local Fill = Create("Frame", {Parent=Box, BackgroundColor3=Library.Theme.Accent, Size=UDim2.new(1,-2,1,-2), Position=UDim2.new(0,1,0,1), Visible=def or false, ZIndex=9})
                Library:RegisterThemeObj("Accent", Fill, "BackgroundColor3"); Create("UICorner", {Parent=Fill, CornerRadius=UDim.new(0,2)})
            ...
            function E:TextBox(txt, ph, flag, cb)
                ...
                local T = Create("TextLabel", {Parent=Box, Text=txt, TextColor3=Library.Theme.Text, Font=Library.Theme.FontSmall, TextSize=12, Size=UDim2.new(1,0,0,15), BackgroundTransparency=1, TextXAlignment=Enum.TextXAlignment.Left, ZIndex=8}); Library:RegisterThemeObj("Text", T, "TextColor3")
                local Input = Create("TextBox", {Parent=Box, BackgroundColor3=Library.Theme.Main, TextColor3=Library.Theme.Text, PlaceholderText=ph, Text="", Size=UDim2.new(1,0,0,20), Position=UDim2.new(0,0,0,20), Font=Library.Theme.FontSmall, TextSize=11, ZIndex=8})
            ...
            function E:ColorPicker(txt, def, flag, cb)
                ...
                local PickFrame = Create("Frame", {Parent=Preview, Visible=false, BackgroundColor3=Library.Theme.Section, Size=UDim2.new(1,0,0,95), Position=UDim2.new(0,0,1,5), ZIndex=20})
                ...
                local function MakeS(t,y,val,set)
                    local SBox = Create("Frame", {Parent=PickFrame, BackgroundTransparency=1, Size=UDim2.new(1,-10,0,25), Position=UDim2.new(0,5,0,y), ZIndex=21})
                    Create("TextLabel", {Parent=SBox, Text=t, TextColor3=Library.Theme.DimText, Size=UDim2.new(0,15,1,0), BackgroundTransparency=1, ZIndex=22})
                    local Bar = Create("Frame", {Parent=SBox, BackgroundColor3=Library.Theme.Main, Size=UDim2.new(1,-20,0,5), Position=UDim2.new(0,20,0.5,-2), ZIndex=22}); Library:RegisterThemeObj("Main", Bar, "BackgroundColor3")
                    local Fill = Create("Frame", {Parent=Bar, BackgroundColor3=Library.Theme.Accent, Size=UDim2.new(val/255,0,1,0), ZIndex=23}); Library:RegisterThemeObj("Accent", Fill, "BackgroundColor3")
                    local Trig = Create("TextButton", {Parent=Bar, BackgroundTransparency=1, Size=UDim2.new(1,0,1,0), Text="", ZIndex=24})
