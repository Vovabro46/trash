--[[
    GILA MONSTER LIBRARY [UPDATED v4]
    Features: TextBox, ColorPicker, Real-time Theme Update
]]
...
local Library = { 
    Flags = {}, 
    Items = {}, 
    ThemeObjects = { Accent = {}, Text = {}, Main = {}, Sidebar = {}, Section = {} }, -- Для обновления цветов
    Theme = {
...
-- Система регистрации объектов для смены темы
...
function Library:Window(opts)
    ...
    local Screen = Create("ScreenGui", {Name = "GilaFixed", Parent = TargetParent})
    local MobToggle = Create("ImageButton", {Parent = Screen, BackgroundColor3 = Library.Theme.Main, Position = UDim2.new(0,50,0,50), Size = UDim2.new(0,45,0,45), Image = Icons.menu, ImageColor3 = Library.Theme.Accent})
    ...
    MakeDraggable(MobToggle, MobToggle)
    ...
    function Tabs:Tab(name)
        ...
        function Funcs:Section(title, side)
            ...
            function E:Slider(txt, min, max, def, flag, cb)
                ...
                local d=false; Trigger.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 then d=true Set(math.floor(min+((max-min)*math.clamp((i.Position.X-Bar.AbsolutePosition.X)/Bar.AbsoluteSize.X,0,1)))) end end)
                UserInputService.InputChanged:Connect(function(i) if d and i.UserInputType==Enum.UserInputType.MouseMovement then Set(math.floor(min+((max-min)*math.clamp((i.Position.X-Bar.AbsolutePosition.X)/Bar.AbsoluteSize.X,0,1)))) end end)
            ...
            function E:ColorPicker(txt, def, flag, cb)
                ...
                local function MakeS(t,y,val,set)
                    ...
                    Trig.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 then d=true In(i) end end)
                    UserInputService.InputChanged:Connect(function(i) if d and i.UserInputType==Enum.UserInputType.MouseMovement then In(i) end end)
                    UserInputService.InputEnded:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 then d=false end end)
                end
